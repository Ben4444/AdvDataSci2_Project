...
############### subset by region and SES of family
...
head(dat)
dat_subset <- dat[dat$topic %in% "abortion",]
length(dat_subset$birthst)

for(i in 1:length(dat_subset$birthst)){
...
}else if(dat_subset$birthst[i] %in% c(52,53,54,55,56,57,58)){
...
plot+geom_bar(stat="identity")+labs(main=c("Number of Opinions Written on","By Birth Region of Justice
              Writing the Opinion and Socioeconomic Status of the Justice's Family"),
...
#### subset by childhood surroundings and SES
dat_subset <- dat[dat$topic %in% "abortion",]
length(dat_subset$birthst)
