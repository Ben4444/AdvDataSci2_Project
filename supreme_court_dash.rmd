...
renderPlot({
  ...
  my_theme <- theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1), panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank(), legend.position = c(1, 1), legend.justification = c(1, 1))
  
 if(!is.null(input$Birth_Region) & is.null(input$Family_Socioeconomic_Status)){

  dat_subset <- dat[dat$region %in% c(input$Birth_Region),] %>% count(topic,famses)
  upper <- max(dat[dat$region %in% c(input$Birth_Region),] %>% count(topic) %>% select(n) + 1)
  ...
  ggplot(dat_subset, aes(x = topic, y = n, fill = famses)) + 
   ...
   dat_subset_1<- dat[dat$famses %in% c(input$Family_Socioeconomic_Status),] %>% count(topic,region)
   upper <- max(dat[dat$famses %in% c(input$Family_Socioeconomic_Status),] %>% count(topic) %>% select(n) + 1)
   
   ggplot(dat_subset_1, aes(x = topic, y = n, fill = region)) + 
     geom_bar(stat = "identity") +
   ...
   upper <- max(dat_subset_3$n) + 1
   
   ggplot(dat_subset_3, aes(x = topic, y = n)) +
     geom_bar(stat = "identity") +
     theme_bw() +
     labels +
     my_theme +
