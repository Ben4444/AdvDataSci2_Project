...
Within the 'By Justice' tab, the top graph shows the number of times a Supreme Court Justice delivered a case opinion. To interact with the bottom graph, one or many Supreme Court Justices may be selected, and a plot will display the case topics the justice(s) delivered the court opiinions for.
...
renderPlot({
  ...
  ggplot(melted_df) + 
    geom_col(aes(x = topic, y = Freq / term, fill = name), position = "dodge") + 
    theme(axis.text.x = element_text(angle = 45, hjust = 1), axis.text = element_text(size = 11), 
...
renderPlot({
  ggplot(dat_sub) + 
    geom_bar(aes(x = name, y = Freq / term, fill = religion, alpha = Party)) + 
    theme(axis.text.x = element_text(angle = 90, hjust = 0.95, vjust = 0.2), axis.text = element_text(size = 11), 
...
renderPlot({
  ...
  melted_df <- as.data.frame(table(dat_subset))
  ggplot(melted_df) + 
    geom_col(aes(x = topic, y = Freq / term, fill = pres_party), position = "dodge") + 
    theme(axis.text.x = element_text(angle = 90, hjust = 0.95, vjust = 0.2), axis.text = element_text(size = 11),
          legend.position = "bottom", panel.background = element_blank(), axis.line = element_line(), axis.title = element_text(size = 15),
          panel.grid.major.y = element_line(color = "grey90")) +
    scale_y_continuous(limits = c(NA,NA), expand = c(0,0)) +
...
renderPlot({
  ...
  melted_df <- as.data.frame(table(dat_subset))
  ggplot(melted_df) + 
    geom_col(aes(x = topic, y = Freq / term, fill = religion), position = "dodge") + 
    theme(axis.text.x = element_text(angle = 90, hjust = 0.95, vjust = 0.2), axis.text = element_text(size = 11),
          legend.position = "bottom", panel.background = element_blank(), axis.line = element_line(), axis.title = element_text(size = 15),
          panel.grid.major.y = element_line(color = "grey90")) +
    scale_y_continuous(limits = c(NA,NA), expand = c(0,0)) +
